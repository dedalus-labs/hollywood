...
- name: Bake release artifact
  run: |
    ...
    sudo artifact-pack \
      --tool-binary "${{ inputs.tool_binary }}" \
...
```typescript
// .github/actions/dcs/dm/package-artifact/src/action.ts
...
## First Real Fixture: artifact packaging Bake

The first hard case is the artifact packaging artifact package. The production path
lives in:
...
| Passwordless `sudo`  | `artifact-pack` creates build directories, package metadata, and release artifacts.                   |
...
validateActionMetadataContent({
	name: ".github/actions/dcs/dm/package-artifact/action.yml",
	content: "name: bake\ndescription: bake\nruns:\n  using: node24\n  main: dist/index.js\n",
});
