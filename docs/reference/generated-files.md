...
re-running it is idempotent. JavaScript action bundles from `hollywood build`
and package bundles from `npm run build` also get generated banners.
...
Run `npx hollywood build` after generation. Commit the bundle with the action,
or build an ignored bundle earlier in every job that invokes the local action.
