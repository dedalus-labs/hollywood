...
import { a as integerInput, c as stringInput, d as summaryText, f as toGitHubName, g as nodeLog, h as nodeFs, i as choiceInput, l as stringOutput, m as nodeExec, n as action, o as pathInput, p as currentRunner, r as booleanInput, s as runAction, t as runGitHubAction, u as summaryCode } from "./github-D7ncFlHr.js";
...
//#endregion
//#region src/paths.ts
const reservedPathDependencyNames = new Set([
	"job",
	"jobId",
	"workflowPaths"
]);
const defaultCheckoutAction = "actions/checkout@df4cb1c069e1874edd31b4311f1884172cec0e10";
const pathDependencies = (jobId, definitions) => {
	assertValidPathDependencyJobId(jobId);
	const normalizedDefinitions = normalizeDefinitions(definitions);
	const refs = /* @__PURE__ */ new Map();
	for (const [name, paths] of Object.entries(normalizedDefinitions)) refs.set(name, {
		changed: eq(needsOutput(jobId, name), "true"),
		name,
		paths
	});
	return Object.assign(Object.fromEntries(refs), {
		job: (options) => pathDependencyJob(jobId, normalizedDefinitions, options),
		jobId,
		workflowPaths: positiveWorkflowPaths(normalizedDefinitions)
	});
};
const matchPathDependency = (path, dependency) => matchesPatternList(normalizePath(path), dependency.paths);
const pathDependencyJob = (jobId, definitions, options = {}) => ({
	name: options.name ?? "Detect changed paths",
	"runs-on": options.runsOn ?? "ubuntu-24.04",
	outputs: Object.fromEntries(Object.keys(definitions).map((name) => [name, stepOutput("detect", name)])),
	steps: [{
		...
		shell: "bash",
		env: {
			BASE_SHA: expr("github.event_name == 'pull_request' && github.event.pull_request.base.sha || github.event.before"),
			HEAD_SHA: expr("github.event_name == 'pull_request' && github.event.pull_request.head.sha || github.sha")
		},
		run: detectorScript(jobId, definitions)
	}]
});
const detectorScript = (jobId, definitions) => [
	"set -euo pipefail",
...
const normalizeDefinitions = (definitions) => {
	for (const [name, paths] of Object.entries(definitions)) {
		...
		if (paths.length === 0) throw new Error(`path dependency ${name} must include at least one path`);
		for (const path of paths) assertValidPathPattern(name, path);
		if (!paths.some((path) => !path.startsWith("!"))) throw new Error(`path dependency ${name} must include at least one positive path`);
	}
	return definitions;
};
const positiveWorkflowPaths = (definitions) => {
	const paths = /* @__PURE__ */ new Set();
	for (const dependencyPaths of Object.values(definitions)) for (const path of dependencyPaths) if (!path.startsWith("!")) paths.add(path);
	return [...paths];
};
const matchesPatternList = (path, patterns) => {
	let matched = false;
	for (const pattern of patterns) {
		const negated = pattern.startsWith("!");
		if (matchesPattern(path, negated ? pattern.slice(1) : pattern)) matched = !negated;
	}
	return matched;
};
const matchesPattern = (path, pattern) => globToRegExp(normalizePath(pattern)).test(path);
const globToRegExp = (pattern) => {
	...
	for (let index = 0; index < pattern.length; index += 1) {
		const char = pattern[index];
		const next = pattern[index + 1];
		if (char === "*" && next === "*") if (pattern[index + 2] === "/") {
			source += "(?:.*/)?";
			index += 2;
		} else {
			source += ".*";
		...
		else if (char !== void 0) source += escapeRegExp(char);
	}
	return new RegExp(`^${source}$`);
};
const normalizePath = (path) => path.replace(/^\/+/, "");
const escapeRegExp = (value) => value.replace(/[.+^${}()|[\]\\]/g, "\\$&");
const assertValidPathDependencyJobId = (jobId) => {
	if (!/^[A-Za-z_][A-Za-z0-9_-]*$/.test(jobId)) throw new Error(`invalid path dependency job id: ${jobId}`);
};
const assertValidPathDependencyName = (name) => {
	if (reservedPathDependencyNames.has(name)) throw new Error(`reserved path dependency name: ${name}`);
	if (!/^[A-Za-z_][A-Za-z0-9_-]*$/.test(name)) throw new Error(`invalid path dependency name: ${name}`);
};
const assertValidPathPattern = (name, path) => {
	if (path.length === 0 || path.trim() !== path || path === "!") throw new Error(`invalid path pattern for ${name}: ${path}`);
};
//#endregion
//#region src/files.ts
