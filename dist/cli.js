...
const createRunnerCommand = (io, services = { probe: probeRunner }) => {
	...
	runner.command("verify").description("Verify a runner probe against a curated contract").argument("<contract>", "Runner contract JSON").argument("<probe>", "Runner probe JSON").action(async (contractPath, probePath) => {
		...
		if (differences.length > 0) throw new Error(`runner contract failed with ${differences.length} difference(s)`);
		io.writeOut("ok	runner contract\n");
	});
	runner.command("compare").description("Classify drift between two runner probes").argument("<expected>", "Reference runner probe JSON").argument("<actual>", "Candidate runner probe JSON").action(async (expectedPath, actualPath) => {
...
const createCli = (io = processIo(), services = { run }) => {
	...
	program.command("run").description("Run an exported Hollywood action locally").argument("<source>", "Source file exporting a Hollywood action").requiredOption("--export <name>", "Action export name").option("--image <reference>", "Digest-pinned runner image", githubActionsRunnerImage).requiredOption("--provider <provider>", "Container provider: container, docker, or podman", parseContainerProvider).option("--with <name=value>", "Action input", collect, []).action(async (source, options) => {
		const runOptions = {
			image: options.image,
			inputs: options.with,
			provider: options.provider,
