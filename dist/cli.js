...
import { basename as basename$1, dirname as dirname$1, isAbsolute as isAbsolute$1, normalize, posix as posix$1, relative as relative$1, resolve as resolve$1, sep as sep$1 } from "path";
...
expr("github.actor"), expr("github.base_ref"), expr("github.event.before"), expr("github.event.pull_request.head.repo.full_name"), expr("github.event_name"), expr("github.head_ref"), expr("github.event.merge_group.base_sha"), expr("github.event.merge_group.head_sha"), expr("github.ref"), expr("github.ref_name"), expr("github.repository_owner"), expr("github.repository"), expr("github.run_id"), expr("github.sha"), expr("github.token"), expr("github.workflow");
...
const parseContainerProvider = (value) => {
	switch (value) {
		case "container":
		case "docker":
		case "podman": return value;
		default: throw new Error(`Unsupported container provider: ${value}.`);
	}
};
const isMissingExecutable$1 = (error) => typeof error === "object" && error !== null && "code" in error && error.code === "ENOENT";
//#endregion
//#region src/container-action.ts
const runContainerAction = async (options) => {
	const directory = await mkdtemp(`${tmpdir()}/hollywood-action-`);
	const bundle = join(directory, "action.mjs");
	try {
		await buildContainerAction(options.source, options.exportName, bundle, options.actionRuntime ?? fileURLToPath(new URL("./action-runtime.js", import.meta.url)));
...
const buildContainerAction = async (source, exportName, outfile, actionRuntime) => {
...
const actionEntrypoint = (source, exportName, actionRuntime) => {
	if (exportName !== "default" && !/^[A-Za-z_$][\w$]*$/.test(exportName)) throw new Error(`Invalid TypeScript export name: ${exportName}.`);
	const actionImport = exportName === "default" ? `import scriptAction from ${JSON.stringify(`./${source}`)};` : `import { ${exportName} as scriptAction } from ${JSON.stringify(`./${source}`)};`;
	return [
...
const inputEnvironment = (inputs) => Object.fromEntries(Object.entries(inputs).map(([name, value]) => [`INPUT_${toGitHubName(name).replaceAll(" ", "_").toUpperCase()}`, value]));
const parseActionResult = (content) => {
	const parsed = JSON.parse(content);
	if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed) || Object.values(parsed).some((value) => typeof value !== "string")) throw new Error("Container action returned invalid outputs. Every output value must be a string.");
	return parsed;
};
//#endregion
...
const assertWorkFolder = (value) => {
	const segments = value.split("/");
	if (value.length === 0 || isAbsolute$2(value) || value.includes("\\") || normalize$1(value) !== value || segments.some((segment) => segment === "." || segment === ".." || segment.length === 0)) throw new Error("GitHub runner work folder must be a relative path without traversal.");
};
...
const createRunnerCommand = (io, services = {
	...
	runner.command("jit-config").description("Create one GitHub repository JIT runner configuration.").argument("<repository>", "GitHub repository in OWNER/REPOSITORY format.").requiredOption("--runner-group-id <id>", "Runner group ID.", parsePositiveInteger).requiredOption("--label <labels...>", "One to 100 unique runner labels.").option("--name <name>", "Runner name.", "hollywood-local").option("--work-folder <path>", "Relative runner work folder.", "_work").option("--output <path>", "New JIT configuration file.", ".hollywood-jit-config").option("--token-env <name>", "Environment variable containing the GitHub token.", "GITHUB_TOKEN").option("--api-url <url>", "GitHub REST API base URL.", parseGitHubApiUrl).action(async (repository, options) => {
		...
		if (token === void 0) throw new Error(`GitHub API token environment variable ${tokenEnvironmentVariable} is not set.`);
	...
	runner.command("listen").description("Run one GitHub job with the official Actions runner.").argument("<jit-config>", "File containing an encoded GitHub JIT configuration.").requiredOption("--provider <provider>", "Container provider: container, docker, or podman.", parseContainerProvider).option("--image <reference>", "Digest-pinned runner image.", githubActionsRunnerImage).option("--container-engine-socket <path>", "Docker-compatible API socket for container jobs and actions.").option("--diagnostics <dir>", "Directory that receives GitHub runner diagnostics.").option("--job-started-hook <path>", "Executable GitHub job-started hook.").option("--job-completed-hook <path>", "Executable GitHub job-completed hook.").option("--container-hook <path>", "Bundled GitHub runner container hook.").option("--require-job-container", "Fail jobs that do not declare a job container.", false).action(async (jitConfigPath, options) => {
		await services.listen({
			...options.containerEngineSocket === void 0 ? {} : { containerEngine: {
				kind: "docker-socket",
			...
			hooks: {
				...options.containerHook === void 0 ? {} : { container: options.containerHook },
				...options.jobCompletedHook === void 0 ? {} : { jobCompleted: options.jobCompletedHook },
				...options.jobStartedHook === void 0 ? {} : { jobStarted: options.jobStartedHook },
	...
	runner.command("probe").description("Write a sanitized runner inventory.").option("-o, --output <path>", "Output JSON path.").action(async (options) => {
		const probe = await services.probe();
		if (options.output === void 0) {
			io.writeOut(`${JSON.stringify(probe, void 0, 2)}\n`);
			return;
		}
		await writeRunnerProbe(options.output, probe);
		io.writeOut(`wrote\t${options.output}\n`);
	});
	runner.command("verify").description("Verify a runner probe against a curated contract.").argument("<contract>", "Runner contract JSON.").argument("<probe>", "Runner probe JSON.").action(async (contractPath, probePath) => {
		const differences = verifyRunner(await readRunnerContract(contractPath), await readRunnerProbe(probePath));
		writeDifferences(io, differences);
		if (differences.length > 0) throw new Error(`Runner contract verification found ${differences.length} differences.`);
		io.writeOut("ok	runner contract\n");
	});
	runner.command("compare").description("Classify differences between two runner probes.").argument("<expected>", "Reference runner probe JSON.").argument("<actual>", "Candidate runner probe JSON.").action(async (expectedPath, actualPath) => {
		const differences = compareRunnerProbes(await readRunnerProbe(expectedPath), await readRunnerProbe(actualPath));
		writeDifferences(io, differences);
		if (differences.length === 0) io.writeOut("ok	identical runner probes\n");
	});
	return runner;
};
const writeDifferences = (io, differences) => {
	for (const difference of differences) io.writeOut(`${difference.category}\t${difference.path}\t${render(difference.expected)}\t${render(difference.actual)}\n`);
};
const render = (value) => value === void 0 ? "(missing)" : JSON.stringify(value);
const parsePositiveInteger = (value) => {
	const parsed = Number(value);
	if (!Number.isSafeInteger(parsed) || parsed <= 0) throw new Error(`Runner group ID must be a positive integer. Received '${value}'.`);
	return parsed;
};
const parseGitHubApiUrl = (value) => {
	try {
...
const parseEnvironmentVariableName = (value) => {
	if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(value)) throw new Error(`GitHub token environment variable name is invalid: '${value}'.`);
	return value;
};
//#endregion
//#region src/commands.ts
const createCli = (io = processIo(), services = { run }) => {
	const program = new Command().name("hollywood").description("Generate, validate, and run typed GitHub Actions.").version(readHollywoodVersion());
	...
	program.command("run").description("Run an exported Hollywood action in a local container.").argument("<source>", "Source file that exports a Hollywood action.").requiredOption("--export <name>", "Action export name.").option("--image <reference>", "Digest-pinned runner image.", githubActionsRunnerImage).requiredOption("--provider <provider>", "Container provider: container, docker, or podman.", parseContainerProvider).option("--with <name=value>", "Action input.", collect, []).action(async (source, options) => {
		const runOptions = {
			image: options.image,
			inputs: options.with,
			provider: options.provider,
...
const run = async (options, io) => {
	const result = await runContainerAction({
		...options.actionRuntime === void 0 ? {} : { actionRuntime: options.actionRuntime },
	...
	if (result.stdout.length > 0) io.writeOut(result.stdout);
	if (result.stderr.length > 0) {
		if (io.writeErr === void 0) throw new Error("container action wrote stderr without a configured stderr sink");
...
const parseInputPairs = (inputs) => {
	...
	for (const input of inputs) {
		...
		if (separator <= 0) throw new Error(`Invalid input '${input}'. Expected name=value.`);
