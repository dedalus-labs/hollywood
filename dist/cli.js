...
import { glob, mkdir, mkdtemp, readFile, readdir, rm, stat, symlink, writeFile } from "node:fs/promises";
...
const globMatches = async (source) => {
	const matches = [];
	for await (const match of glob(source)) matches.push(match);
	return matches;
};
